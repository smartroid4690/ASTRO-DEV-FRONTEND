...
const Form = () => {
	...
	useEffect(() => {
		...
		const fetchTestCategories = async () => {
			const testCategories = await getTestCateogories();
	...
	const handleBaseAlloyChange = async (e) => {
		setValue("base_metal_alloy", e.value.name);

		try {
			const alloys = await getAlloysByBaseAlloyId(e.value.id);
			setAlloys(alloys);
		} catch (error) {
			console.log("Error fetching alloys", error.message);
		}
	};
